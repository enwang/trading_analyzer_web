 ...
 * (e.g. one at midnight UTC and one at market open UTC). Keep the later timestamp
 * which is more likely to be the actual session candle.
 */
export function deduplicateDailyCandles(candles: Candle[]): Candle[] {
  ...
  for (const c of candles) {
    ...
    if (!existing || c.time > existing.time) byDate.set(date, c)
  }
